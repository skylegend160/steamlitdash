...
try: 
    ...
    st.error("CSV file 'airockfalldata.csv' not found. Please ensure the file is in the same directory.")
    st.stop()
...
rainfall_cols = [m + 'r' for m in months_list]
temp_cols = [m + 't' for m in months_list]

col1, col2, col3, col4 = st.columns([1.5, 1.5, 1.5, 2])
with col1:
    st.metric("Sample Count", len(filtered_data))
with col2:
    avg_rain = filtered_data[rainfall_cols].mean().mean()
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["Rainfall & Temperature", "Elevation & Terrain", "Rockfall Insights", "Data Table", "Data Download"]
)
...
if JS_AVAILABLE and not user_location:
    st.warning("⚠ Please allow location access in your browser to enable auto-detect of your location.")
elif JS_AVAILABLE and auto_location is None:
    st.warning("⚠ Your detected location does not match a known zone, please pick manually from the sidebar.")
elif not JS_AVAILABLE:
    st.info("ℹ Install 'streamlit-javascript' and 'geopy' packages to enable automatic geolocation detection.")